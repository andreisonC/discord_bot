...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):
        ...
        if message.content == '!sexo':
            await message.channel.send(f'@{message.author.name}, só mais tarde')
        elif message.content == '<@!870666144426627092>':
            await message.author.send(f'Oi, precisa de mim? Cheque meus comandos:{os.linesep}--------  Prefixo ! ---------- {os.linesep}!regras {os.linesep}!nivel')
